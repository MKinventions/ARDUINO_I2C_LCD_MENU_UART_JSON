...
const int pagesButtonIncrease = 12;//PB15;
const int pagesButtonDecrease = 2;//PB15;
const int MainMenuButton = 4;//PB13;
const int SubMenuButton = 9;//PB12;
...
void setup() {
  ...
  pinMode(pagesButtonIncrease, INPUT_PULLUP);
  pinMode(pagesButtonDecrease, INPUT_PULLUP);
...
//    doc["port"] = "8080";
    ...
    JsonArray button = doc.createNestedArray("switch");
    button.add(sw1Dir_value);
    button.add(sw2Dir_value);
    button.add(sw3Dir_value);
    button.add(sw4Dir_value);
...
void loop() {
  ...
  pot3Dir_Type = (pot3Dir_value == 0)?"L":"R";
  pot4Dir_Type = (pot4Dir_value == 0)?"L":"R";
  
  joy1Dir_Type = (joy1Dir_value == 0)?"L":"R";
  joy2Dir_Type = (joy2Dir_value == 0)?"L":"R";
  joy3Dir_Type = (joy3Dir_value == 0)?"L":"R";
  joy4Dir_Type = (joy4Dir_value == 0)?"L":"R";
  
  sw1Dir_Type = (sw1Dir_value == 0)?"PUSH":"TOGL";
  sw2Dir_Type = (sw2Dir_value == 0)?"PUSH":"TOGL";
  sw3Dir_Type = (sw3Dir_value == 0)?"PUSH":"TOGL";
  sw4Dir_Type = (sw4Dir_value == 0)?"PUSH":"TOGL";
 ...
 if (digitalRead(MainMenuButton) == LOW) {
    ...
    if (digitalRead(pagesButtonIncrease) == LOW) {
      pagesCount++;
    ...
    if (digitalRead(pagesButtonDecrease) == LOW) {
      pagesCount--;
      if (pagesCount <= 0) {
        pagesCount = 0;
      }
    }
//    Serial.println(pagesCount);
...
switch(pagesCount){
    ...
    case 1:EEPROM.write(pageCount_address, 1);
      lcd.setCursor(0,0); lcd.print("P1:" + String(pot1_value)+ String(pot1Dir_Type));lcd.setCursor(7,0);lcd.print(",P2:" + String(pot2_value)+ String(pot2Dir_Type));
      lcd.setCursor(0,1); lcd.print("P3:" + String(pot3_value)+ String(pot3Dir_Type));lcd.setCursor(7,1);lcd.print(",P4:" + String(pot4_value)+ String(pot4Dir_Type));
      lcd.setCursor(15,1);lcd.print(String(pagesCount));
      break;
    case 2:EEPROM.write(pageCount_address, 2);
      lcd.setCursor(0,0); lcd.print("J1:" + String(joy1_value)+ String(joy1Dir_Type));lcd.setCursor(7,0);lcd.print(",J2:" + String(joy2_value)+ String(joy2Dir_Type));
      lcd.setCursor(0,1); lcd.print("J3:" + String(joy3_value)+ String(joy3Dir_Type));lcd.setCursor(7,1);lcd.print(",J4:" + String(joy4_value)+ String(joy4Dir_Type));
      lcd.setCursor(15,1);lcd.print(String(pagesCount));
      break;
...
void potentiometer(){
  ...
  switch(MainMenuSelect){
         case 0:
                lcd.setCursor(0,0);lcd.print(" P1  P2  P3  P4");
                lcd.setCursor(0,1);lcd.print(" 180 255 L R");
                break;
...
void switch_settings(){
  ...
  switch(MainMenuSelect){
         case 0:
                lcd.setCursor(0,0);lcd.print(" S1  S2  S3  S4");
                lcd.setCursor(0,1);lcd.print(" PUSH  TOGGLE");
                break;
